...
const BottomBoxItem = ({ blok }) => {
  ...
  return (
    <div
      ...
      key={blok._uid}
      className=" relative grow-1 shrink-1 basis-auto lg:flex-1"
    >
      <Link to={link}>
        <img
          src={`${image?.filename}`}
        ...
        <div className="box-content max-w-[310px] pt-[200px] pb-[30px] px-[35px] text-xs leading-[1.4] absolute inset-0 [&>a]:text-white">
          ...
          <div className="box-over opacity-0 hover:opacity-100 transition duration-300">
        ...
        <div className="box-overlay absolute inset-0 w-full h-full opacity-0 hover:opacity-50 z-[-1]  transition duration-300 bg-primary"></div>
      </Link>
    </div>
  );
};
